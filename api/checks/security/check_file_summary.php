...
function check_file_summary(array $tree, array $languages): array
{
    ...
    // Count Claude-style skills by package directories containing SKILL.md.
    // Supported layouts:
    ...
    foreach ($tree as $node) {
        ...
        if (preg_match('#^(?:\.claude/)?skills/([^/]+)/SKILL\.md$#i', $path, $m) === 1) {
            $slug = trim((string) $m[1]);
            if ($slug !== '') {
                $skillPackages[strtolower($slug)] = $slug;
            }
        }
    }

    if (!empty($skillPackages)) {
        natcasesort($skillPackages);
        foreach ($skillPackages as $slug) {
            $display = str_replace(['-', '_'], ' ', $slug);
            $skills[] = [
                'skill_name'        => 'Claude Skill: ' . ucwords($display),
                'proficiency_level' => 'Skill Package',
                'risk_level'        => 'Low',
            ];
        }

        $skillsCount = count($skillPackages);
        $findings[] = [
            'category'    => 'File Summary',
            'title'       => "Detected {$skillsCount} Claude skill package" . ($skillsCount === 1 ? '' : 's'),
            'description' => 'Skill count is based on unique directories that contain SKILL.md under skills/ or .claude/skills/.',
            'severity'    => 'Low',
        ];
    } else {
        // Fallback: build language skills when explicit skill packages are not present.
    ...
    foreach ($skills as $skill) {
        $key = strtolower(trim((string) ($skill['skill_name'] ?? '')));
        if ($key === '' || isset($dedupedSkills[$key])) {
            continue;
        }
        $dedupedSkills[$key] = [
            'skill_name' => (string) ($skill['skill_name'] ?? ''),
            'proficiency_level' => (string) ($skill['proficiency_level'] ?? 'Supporting Skill'),
            'risk_level' => (string) ($skill['risk_level'] ?? 'Low'),
        ];
    }
    $skills = array_values($dedupedSkills);
